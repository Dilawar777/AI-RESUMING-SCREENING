...
if analyze_btn:
    if not uploaded_file:
        ...
        with tab1:
            st.markdown('<p class="section">Keywords to Add to Your Resume</p>', unsafe_allow_html=True)
            if missing:
                cols = st.columns(3)
        ...
        with tab2:
            st.markdown('<p class="section">Skills Your Resume Already Has</p>', unsafe_allow_html=True)
            if matched:
                cols = st.columns(3)

...
st.markdown('<p style="font-size:1rem;color:#AAAAAA;margin-top:-10px">Upload your resume · Paste any job description · Get an instant match score</p>', unsafe_allow_html=True)
st.markdown("")
...
with col1:
    st.markdown('<p style="font-size:1.1rem;font-weight:600;color:#4DA3FF;border-bottom:2px solid #1A56A0;padding-bottom:4px;margin:20px 0 12px">Upload Resume (PDF)</p>', unsafe_allow_html=True)
...
if analyze_btn:
    if not uploaded_file:
        ...
        if final_score >= 75:
            ...
            st.markdown(f'<div class="moderate">⚠️ <b>MODERATE MATCH ({final_score:.0f}%)</b> — Good foundation but consider adding missing keywords to improve your chances.</div>', unsafe_allow_html=True)
        else:
        ...
        with tab1:
            st.markdown('<p style="font-size:1.1rem;font-weight:600;color:#FF6B6B;border-bottom:2px solid #DC2626;padding-bottom:4px;margin:20px 0 12px">Keywords to Add to Your Resume</p>', unsafe_allow_html=True)
            if missing:
                cols = st.columns(3)
        ...
        with tab2:
            st.markdown('<p style="font-size:1.1rem;font-weight:600;color:#51CF66;border-bottom:2px solid #059669;padding-bottom:4px;margin:20px 0 12px">Skills Your Resume Already Has</p>', unsafe_allow_html=True)
            if matched:
                cols = st.columns(3)
        ...
        with tab3:
            st.markdown('<p style="font-size:1.1rem;font-weight:600;color:#4DA3FF;border-bottom:2px solid #1A56A0;padding-bottom:4px;margin:20px 0 12px">Resume Skills by Category</p>', unsafe_allow_html=True)
